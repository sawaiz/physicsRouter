...
native v1.6 atomic/pad-aware bundle router and the board-derived fab profile. Source:
...
|--------|---------|----------------|
| Completion | **17/23 nets** (6 honestly open) | **7/7 nets** |
| Segments / length | 164 · 464.3 mm | 41 · 93.6 mm |
| Vias / copper areas | 0 / **2** | 0 / **3** |
| **Native router DRC total** | **0** | **0** |
| → shorts / spacing | 0 / 0 | 0 / 0 |
| → **outside Edge.Cuts** | **0** | 0 |
| Bends / multi-bend nets | 160 · 11 nets | 35 · 4 nets |
| Clearance route time | ~7 s | ~0.06 s |
...
4. **Bounded matrix rebuild helps** — four parallel native order variants raise legal HALO completion from 15 to 17 nets without sequential peer-rip thrashing.
5. **The remaining problem is explicit** — six CPX nets still need a genuinely concurrent/ring-topology solver. The router stops after a bounded bundle instead of spending minutes thrashing or claiming illegal completion.
6. **This is not fabrication sign-off** — native DRC checks tracks/vias and zone boundaries; `kicad-cli pcb drc --refill-zones` remains authoritative for the final filled polygons and thermals.
...
| Lesson retained | Native v1.6 response |
|-----------------|----------------------|
