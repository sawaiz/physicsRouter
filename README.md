...
native v1.7 pad-layer-legal bundle router and the board-derived fab profile. Source:
...
| Segments / length | 144 · 265.1 mm | 106 · 190.2 mm |
| Vias / copper areas | **21** / **1** | **4** / **3** |
| **Native router DRC total** | **0** | **0** |
| → shorts / spacing | 0 / 0 | 0 / 0 |
| → **outside Edge.Cuts** | **0** | 0 |
| Bends / multi-bend nets | 161 · 9 nets | 90 · 7 nets |
| Clearance route time | ~3.2 s | ~0.05 s |
...
5. **Bounded rebuild helps without hiding failure** — parallel power, critical, and matrix order variants complete 9 nets, including CPX-1 and CPX-5, in about 3.2 seconds.
6. **The remaining problem is explicit** — 14 nets, including eight CPX nets, need a genuinely concurrent bundle/topology solver. The old 17/23 figure is retired because it counted electrically unreachable inner-layer endpoints.
7. **This is not fabrication sign-off** — native DRC checks tracks/vias and zone boundaries; `kicad-cli pcb drc --refill-zones` remains authoritative for the final filled polygons and thermals.
...
| Lesson retained | Native v1.7 response |
|-----------------|----------------------|
...
# → 1.7.0-pad-legal-bundle · oriented pads · layer-reachable anchors · copper areas
```
