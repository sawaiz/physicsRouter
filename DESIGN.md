...
**Why:** Two parallel router implementations drifted. Native v1.9 evaluates bounded bucket orders and board-wide negotiated candidates concurrently. Under strict pad-layer reachability and exact conflict legalization, the documented HALO snapshot completes 12/23 nets with zero native DRC violations and zero generated-copper KiCad errors. The earlier 17/23 native number was invalid because inner-layer tracks at front-only SMD pads were counted as connected without vias; the Python multipin experiment reached 19/23 under its older model but took 625 seconds.
