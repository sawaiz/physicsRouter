...
def plan_graph_topology(
    ...
    *,
    ...
    initial = {name: minimum_spanning_tree(edge) for name, edge in hyperedges.items()}
    ...
    for name in sorted(hyperedges, key=lambda n: (-hyperedges[n].priority, n)):
        ...
        refined[name] = minimum_spanning_tree(
            hyperedges[name],
...
def analyze_route_graph(result: Any) -> dict[str, Any]:
    ...
    for net, net_segments in by_net.items():
        ...
        for via in (value for value in vias if value.net == net):
            layers = tuple(via.layers or ())
            if len(layers) < 2:
                continue
            a = node(via.x, via.y, layers[0])
            b = node(via.x, via.y, layers[-1])
            edge_pairs.append((a, b))
            via_edges += 1
            degree[a] = degree.get(a, 0) + 1
            degree[b] = degree.get(b, 0) + 1
